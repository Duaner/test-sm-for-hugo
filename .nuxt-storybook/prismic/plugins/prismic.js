...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://test-sm1512.cdn.wroom.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"page","path":"/:uid"}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://test-sm1512.cdn.wroom.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=test-sm1512.cdn.wroom.io/api/v2&new=true'
  ...
  if (process.client && process.static && route.path !== '/preview') {
    ...
    const repo = 'test-sm1512.cdn.wroom.io/api/v2'
